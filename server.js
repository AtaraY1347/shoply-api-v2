...
/* הכל חיצוני רק עם הגבלת מוצרים עובד
...
async function fetchRealPricesForProduct(productCode) {
    ...
    try {
        ...
        storeKeys.forEach((key, idx) => {
            const variation = ((seed + idx * 37) % 30) - 15;
            const storePrice = avg * (1 + variation / 100);
            storePrices[key] = Math.round(storePrice * 10) / 10;
        });

        setCached(cacheKey, storePrices, 3 * 60 * 60 * 1000);
        return storePrices;
...
// ============================================================
//  📡 API חיצוני 2: Open Food Facts
//  ✅ שינוי: pagination מלא — שולף את כל המוצרים ללא הגבלה
// ============================================================
async function offToProduct(offProduct, fallbackCategory) {
    let englishName = "";
    const candidates = [
...
// ✅ שינוי: pagination מלא — מביא את כל העמודים ללא הגבלת כמות
async function fetchByOFFCategory(offTag, fallbackHe, pageSize = 200) {
    const fields = "code,product_name,product_name_en,generic_name,quantity,categories_tags";
    const tagOnly = offTag.replace("en:", "");
    const results = [];
    let page = 1;
    ...
    while (true) {
        const url = `${OFF_BASE}/api/v2/search?categories_tags_en=${encodeURIComponent(tagOnly)}`
            + `&fields=${fields}&page_size=${pageSize}&page=${page}&sort_by=popularity_key`;

        console.log(`  📡 [OFF] Fetching: ${offTag} — page ${page}`);
        ...
        try {
            ...
            // אם חזרו פחות תוצאות מהמבוקש — זה העמוד האחרון
            if (products.length < pageSize) break;

            page++;
            await sleep(2000); // המתנה בין עמודים
        } catch (err) {
            console.error(`  ✗ [OFF] Failed [${offTag}] page ${page}:`, err.message);
            break;
        }
    }
...
async function fetchAllProducts() {
    ...
    // ── שלב 2: Open Food Facts ───────────────────────────────
    // ✅ שינוי: הוסר categoryCount — אין יותר הגבלה על מספר המוצרים לקטגוריה
    console.log("→ Step 2: Open Food Facts (packaged products)");
    ...
    for (const [heCategory, offTags] of uniqueByHe) {
        for (const offTag of offTags) {
            const products = await fetchByOFFCategory(offTag.offTag, heCategory);
...
function toApiFormat(product) {
    return {
        ...
        isSelected: false,
        isPurchased: false,
        priceSource: product.priceSource || "estimated",
        storePrices: Object.entries(product.prices).map(([key, price]) => ({
            storeName: STORE_NAMES[key],
            price: price,
        })),
